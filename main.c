...
Recipe* add_recipe( char *name ){
    ...
    temp = (Recipe*) malloc( sizeof(Recipe) ); //casting prima di malloc
...
void remove_recipe(char *name, Order* order_list_head, Order*  ready_list_head ){
    ...
    //check non usata da ordini in sospeso
    while( var != NULL) {
...
int check_can_cook( Recipe* recipe, int multiplier, int current_time){
    ...
    while(ingr != NULL ) {
        ...
        // while per togliere lotti scaduti
...
void remove_lots( Recipe* rem_rec, int multiplier ) {
    ...
    while(ingr != NULL ) {
        ...
        // all'inizio tolgo gli elementi scaduti?
        // no li ho già tolti in check can cook che chiamo allo stesso istante di tempo
